...
Write-Host "Construyendo GamuCare AI 0.15.0..."
...
Write-Host "Creando o normalizando la cuenta tecnica..."
Invoke-Compose -Arguments @("exec", "-T", "api", "python", "-m", "app.scripts.ensure_technical_user") | Out-Null

Write-Host "Version de migracion aplicada:"
Invoke-Compose -Arguments @("exec", "-T", "api", "alembic", "current") | Out-Null

if ($DownloadSources) {
    Write-Host "Descargando fuentes oficiales externas..."
    Invoke-Compose -Arguments @("exec", "-T", "api", "python", "-m", "app.scripts.download_rag_sources") | Out-Null
}

if (-not $SkipRagReindex) {
    Write-Host "Reconstruyendo la coleccion documental de Qdrant..."
...
if ($RunPermissionMatrix) {
    Write-Host "Validando la matriz de permisos de los cuatro perfiles..."
    & "$PSScriptRoot/test-permissions.ps1"
}

if ($IncludeResilience) {
    Write-Host "Ejecutando la prueba controlada de resiliencia..."
    & "$PSScriptRoot/resilience-test.ps1"
}

Write-Host "Actualizacion 0.15.0 completada." -ForegroundColor Green
Write-Host "Aplicacion:        http://localhost:8080"
Write-Host "Perfil tecnico:    technical@gamucare.local"
Write-Host "Estado tecnico:    http://localhost:8080/system-status"
Write-Host "Grafana:           http://localhost:3000"
Write-Host "Prometheus:        http://localhost:9090"
Write-Host "Alertmanager:      http://localhost:9093"
Write-Host "Validacion final:  .\scripts\release-validation.ps1 -IncludeMonitoring"
Write-Host "Permisos:          .\scripts\test-permissions.ps1"
Write-Host "Resiliencia:       .\scripts\resilience-test.ps1"
Write-Host "Fuentes RAG:       .\scripts\download-rag-sources.ps1 -Reindex"
